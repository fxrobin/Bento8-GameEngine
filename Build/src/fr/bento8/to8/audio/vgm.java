...
public class vgm{
	...
	public static void main(String[] args) throws Throwable {
		...
		byte[] file_signature_b = new byte[4];
		int file_signature;
		byte[] frame_rate_b = new byte[4];
		int frame_rate;
		int sample_divider=735;                            // NTSC (default)

		System.out.println("*** Sverx's VGM to PSG converter (by Bentoc) ***\n");
		...
		file_signature_b = fIN.readNBytes(4);
		file_signature = fromByteArray(file_signature_b);
		...
		fIN.skipNBytes(VGM_HEADER_FRAMERATE);
		fIN.read(frame_rate_b, 0, 4); // read frame_rate in the VGM header
		frame_rate = fromByteArray(frame_rate_b);
		if (frame_rate==60) {
		...
		fIN.skipNBytes(VGM_HEADER_LOOPPOINT);
		fIN.read(loop_offset_b, 0, 4); // seek to LOOPPOINT in the VGM header
		loop_offset = fromByteArray(loop_offset_b); // read loop_offset

		fIN.skipNBytes(VGM_DATA_OFFSET);
		fIN.read(data_offset_b, 0, 4); // seek to DATAOFFSET in the VGM header
		data_offset = fromByteArray(data_offset_b); // read data_offset
		...
		fIN.skipNBytes(data_offset);

		if (loop_offset!=0) {
			System.out.format("Info: loop point at 0x%08x\n",loop_offset);
		...
		while ((!leave)) {
			...
			default:
				System.out.format("Fatal: found unknown char 0x%02x\n",c);
				leave = true;
				fatal = true;
				break;
			}
