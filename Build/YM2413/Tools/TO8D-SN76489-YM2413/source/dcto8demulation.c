...
// start sn76489 emulation

int sound,soundt;           //niveau du haut-parleur
int voc[4];
unsigned int freq[4]={1023,1023,1023,1023};
int vol[4]={0,0,0,0};
int tick[4];
int noise_typ=0;
int noise_index1=0;
...
int Run(int ncyclesmax)
{
 ...
 while(ncycles < ncyclesmax)
 {
  ...
  if(tick[v]>(freq[v]*8.94))
  {
      ...
      {   tick[v]=0;
          noise_index1=(noise_index1+1)%noise_size1;
          noise_index2=(noise_index2+1)%noise_size2;
          if(noise_typ==0)
          voc[v]=(noise1[noise_index1>>3]>>(7-(noise_index1%8)))&0x01;
          else
           voc[v]=(noise2[noise_index2>>3]>>(7-(noise_index2%8)))&0x01;
...
void sn76489(char c)
{
    ...
    if(cu>>7)      //latch or modify
    {
        chan=(cu>>5)&0x03;   //  chan select
        if((cu>>4)&0x01)
        {
            vol[chan]=15-(cu&0x0f);
    ...
    else
    {
		...
		{
			freq[chan]&=0x00f;
    		freq[chan]|=((unsigned int)((cu&0x3f)<<4));
    			if(noise_ch2) freq[3]=freq[2];
		}
		else
		{
